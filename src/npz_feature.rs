...
use crate::write::{write_options, WriterBuilder};
...
impl<R: io::Read + io::Seek> NpzArchive<R> {
    ...
    pub fn new(reader: R) -> io::Result<Self> {
        Ok(NpzArchive {
            zip: zip::ZipArchive::new(reader).map_err(invalid_data)?,
        })
    }
    ...
    pub fn array_names(&self) -> impl Iterator<Item = &str> {
        self.zip
            .file_names()
            .filter_map(crate::npz::array_name_from_file_name)
    }
    ...
    pub fn by_name<'a>(
        &'a mut self,
        name: &str,
    ) -> io::Result<Option<NpyFile<zip::read::ZipFile<'a>>>> {
        match self
            .zip
            .by_name(&crate::npz::file_name_from_array_name(name))
        {
...
impl<W: io::Write + io::Seek> NpzWriter<W> {
    ...
    pub fn array<T: Serialize + ?Sized>(
        &mut self,
        name: &str,
        options: zip::write::FileOptions,
    ) -> io::Result<NpzWriterBuilder<'_, T, W>> {
        self.zip
            .start_file(crate::npz::file_name_from_array_name(name), options)?;
...
pub type NpzWriterBuilder<'w, T, W> =
    write_options::WithWriter<&'w mut zip::ZipWriter<W>, write_options::WriteOptions<T>>;
