...
macro_rules! gen_benches {
    ...
    ($read_to_vec_testname:ident, $write_testname:ident, $T:ty, $new:expr) => {
        ...
        fn $read_to_vec_testname(b: &mut Bencher) {
            ...
            b.iter(|| bb(npyz::NpyData::<$T>::from_bytes(&bytes).unwrap().to_vec()));
        }

        fn $write_testname(b: &mut Bencher) {
            b.iter(|| bb(write_array_via_push()));
        }
...
mod simple {
    ...
    gen_benches!(simple_read_to_vec, simple_write, Simple, |i| Simple {
        a: i as i32,
        b: i as f32
    });
}
...
mod one_field {
    ...
    gen_benches!(one_field_read_to_vec, one_field_write, OneField, |i| {
        OneField { a: i as i32 }
    });
}
...
mod array {
    ...
    gen_benches!(array_read_to_vec, array_write, Array, |i| Array {
        a: [i as f32; 8]
    });
}
...
mod plain_f32 {
    ...
    gen_benches!(plain_f32_read_to_vec, plain_f32_write, f32, |i| i as f32);
}
